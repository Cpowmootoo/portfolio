...
function ProjectCard({p,i,onOpen}:{p:typeof projects[number];i:number;onOpen:(project:typeof projects[number])=>void}) {
 const content=<><div className={'project-visual '+(p.fit||'')}><img src={p.image} alt={p.alt} loading="lazy" decoding="async"/><span className="project-count">{String(i+1).padStart(2,'0')} / {projects.length}</span><span className="visit">{'caseStudy' in p?'View case study':'View website ↗'}</span></div><div className="project-caption"><div><span className="eyebrow">{p.category}</span><h3>{p.name}</h3></div><span className="project-arrow" aria-hidden="true">{'caseStudy' in p?'＋':'↗'}</span></div></>;
 return <article className={'project '+p.key}>{'caseStudy' in p?<button className="project-trigger" type="button" onClick={()=>onOpen(p)} aria-haspopup="dialog" aria-label={'View '+p.name+' case study'}>{content}</button>:<a href={p.url} target="_blank" rel="noopener noreferrer" aria-label={'Visit '+p.name+' (opens in a new tab)'}>{content}</a>}<p className="project-description">{p.description}</p></article>
}
function ProjectModal({project,onClose}:{project:typeof projects[number]|null;onClose:()=>void}) {
 ...
 if(!project||!('caseStudy' in project))return null;
...
export default function Home(){const [activeProject,setActiveProject]=useState<typeof projects[number]|null>(null);return <>
